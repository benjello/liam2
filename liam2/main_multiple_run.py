...
def simulate_replications(args):
    replications = args.replications
    print("Using simulation file: '{}' to run {} replications".format(args.fpath, replications))

    for seed in range(replications):
        print(seed)
        output_path = os.path.join(args.output_path, 'replication' + '_' + str(seed))
        print(output_path)
        simulation = Simulation.from_yaml(args.fpath,
                                          input_dir=args.input_path,
                                          input_file=args.input_file,
...
def main():
    ...
    parser_run.add_argument('--output-path', dest='output_path',
                            help='override the output path')
    ...
    parser_run.add_argument('--replications', type=int,
                            help='number of replications to simulate (integer)')
    ...
    if action == 'run':
        if parsed_args.replications > 1:
            func, args = simulate_replications, (parsed_args,)
        else:
